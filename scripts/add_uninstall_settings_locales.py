...
def main():
    ...
    locale_values = {
        'en_us': en_values, 'en_uk': en_values,
        'de_de': de_values, 'de_ch': de_values,
        'es_es': {
    ...
    for file in locales_dir.glob("*.json"):
        ...
        if "settings" not in data or not isinstance(data["settings"], dict):
            data["settings"] = {}
        if "instance" not in data["settings"] or not isinstance(data["settings"]["instance"], dict):
            data["settings"]["instance"] = {}

        locale = file.stem
        values = locale_values.get(locale, en_values)
        
        changed = False
        for k, v in values.items():
            if k not in data["settings"]["instance"] or (locale not in ["en_us", "en_uk"] and data["settings"]["instance"][k] == en_values.get(k)):
                data["settings"]["instance"][k] = v
                changed = True
    ...
    print(f"[locales] Updated {touched} locale file(s).")
