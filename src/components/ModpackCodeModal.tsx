...
function ModpackCodeModal({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="p-0 gap-0 overflow-hidden flex flex-col max-h-[90vh] sm:max-w-lg border-border">
          ...
          {mode === "export" && (
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${!exportedCode && mode === "export" ? "bg-primary text-black" : " bg-card text-muted-foreground hover:bg-accent"}`}
              >
                New Export
              </button>
              <button
                ...
                className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${mode === "my-codes" ? "bg-primary text-black" : " bg-card text-muted-foreground hover:bg-accent"}`}
              >
              ...
              {exportedCode ? (
                <div className="text-center py-8">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 px-6 py-3 bg-card hover:bg-accent rounded-xl text-foreground font-bold transition-colors flex items-center justify-center gap-2"
                    >
                      <svg
                    ...
                    <button
                      ...
                      className="px-6 py-3 bg-card hover:bg-accent rounded-xl text-foreground font-bold transition-colors"
                    >
                      Back
                    </button>
                  </div>
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      {[
                        { id: "mods", label: "Mods", count: mods.length },
                        {
                          ...
                          count: resourcePacks.length,
                        },
                        { id: "shaders", label: "Shaders", count: shaders.length },
                      ].map((type) => (
                        <label
                          key={type.id}
                          className="flex items-center gap-3 p-3 bg-card rounded-xl border border-border cursor-pointer hover:bg-accent transition-colors group"
                        >
                          <div
                            className={`w-6 h-6 rounded-lg flex items-center justify-center border-2 transition-all ${selectedTypes[type.id] ? "bg-primary border-primary" : "border-border"}`}
                          >
                          ...
                          <input
                            ...
                            className="hidden"
                          />
                          <span className="flex-1 text-sm font-bold text-foreground transition-colors">
                            {type.label}
                          </span>
                          <span className="text-xs text-muted-foreground font-bold bg-card/60 px-2 py-1 rounded-md">
                            {type.count}
                          </span>
                        </label>
                      ))}
                  ...
                  <button
                    ...
                    className="w-full py-4 bg-primary hover:bg-primary-hover text-black font-bold rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                  >
                    {loading ? (
                      <>
            ...
            <div className="space-y-2">
              {myCodes.length === 0 ? (
                ...
                myCodes.map((item) => (
                  <div
                    key={item.code}
                    className="bg-card border border-border rounded-xl p-4 flex items-center justify-between group hover:bg-accent transition-all"
                  >
                    <div>
                      <div className="text-lg font-mono font-bold text-primary mb-1">
                        {item.code}
                      </div>
                      <div className="text-xs font-bold text-foreground truncate max-w-[200px] mb-1">
                        {item.name}
                      </div>
              ...
              <button
                ...
                className="w-full mt-4 py-3 bg-card hover:bg-accent text-foreground text-xs font-bold uppercase tracking-widest rounded-xl transition-all border border-border"
              >
                Back to Export
              </button>
            </div>
            ...
            <>
              <div className="mb-6">
                ...
                <input
                  ...
                  maxLength={8}
                  className="w-full bg-card border border-border rounded-xl p-4 text-foreground font-mono text-center text-3xl tracking-widest focus:border-primary outline-none transition-all"
                  autoFocus
                />
