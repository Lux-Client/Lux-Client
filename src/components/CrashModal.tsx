...
import { applyVisibilityFilters } from "../utils/instanceTypes";
...
const CrashModal = ({ isOpen, onClose, crashData, onFixApplied }) => {
  ...
  useEffect(() => {
    ...
    const resolveCrashLogContent = async () => {
      const directLog =
        typeof crashData?.logContent === "string" ? crashData.logContent : "";
      if (directLog.trim()) return directLog;

      const inlineFallback =
        typeof crashData?.logFile === "string" ? crashData.logFile : "";
      ...
      try {
        const logResponse = await window.electronAPI.getLog(
          instanceName,
          "latest.log",
        );
  ...
  const hasCompatibilityIssues = issues.some(
    (issue) => issue.fixAction === "install_compatible_mod",
  );
  ...
  const scoreProjectMatch = (project, targetName) => {
    const normalizedTarget = normalizeToken(targetName).replace(
      /[^a-z0-9]+/g,
      "",
    );
    if (!normalizedTarget) return 0;

    const title = normalizeToken(project?.title || "").replace(
      /[^a-z0-9]+/g,
      "",
    );
    const slug = normalizeToken(project?.slug || "").replace(/[^a-z0-9]+/g, "");
    const projectId = normalizeToken(project?.project_id || "").replace(
      /[^a-z0-9]+/g,
      "",
    );
  ...
  const autoInstallCompatibleMod = async (issue) => {
    ...
    const requestedModName =
      compatibility.targetMod ||
      compatibility.dependencyName ||
      compatibility.modName;
    const instanceName = String(crashData?.instanceName || "").trim();
    ...
    const filteredInstances = applyVisibilityFilters(instances, settings);
    const instance = Array.isArray(filteredInstances)
      ? filteredInstances.find(
          (entry) =>
            normalizeToken(entry?.name) === normalizeToken(instanceName),
        )
      : null;
    ...
    if (!instance) {
      return {
        success: false,
        error: `Instance "${instanceName}" was not found.`,
      };
    }
    ...
    if (
      ...
      return {
        success: false,
        error: `No matching mod found for "${requestedModName}".`,
      };
    }

    const sortedCandidates = [...searchRes.results].sort(
      (a, b) =>
        scoreProjectMatch(b, requestedModName) -
        scoreProjectMatch(a, requestedModName),
    );
    const selectedProject = sortedCandidates[0];
    if (!selectedProject?.project_id) {
      return {
        success: false,
        error: "No valid project ID returned by search.",
      };
    }
    ...
    if (
      !versionsRes?.success ||
      !Array.isArray(versionsRes.versions) ||
      versionsRes.versions.length === 0
    ) {
      return {
        success: false,
    ...
    const file =
      selectedVersion.files?.find((entry) => entry.primary) ||
      selectedVersion.files?.[0];
    if (!file?.url || !file?.filename) {
      return {
        success: false,
        error:
          "Compatible version was found but no downloadable file is available.",
      };
    }
  ...
  const handleApplyFix = async (issue) => {
    ...
    setFixStatus({
      type: "info",
      message: t("crash.applying_fix", { fix: t(issue.fixText) }),
    });
    ...
    try {
      ...
      switch (issue.fixAction) {
        case "increase_memory":
          res = await window.electronAPI.updateInstanceConfig(
            crashData.instanceName,
            { maxMemory: 4096 },
          );
          break;
        case "fix_java_version":
          res = await window.electronAPI.reinstallInstance(
            crashData.instanceName,
            "java",
          );
          break;
        ...
        case "install_compatible_mod":
          ...
          break;
        case "reset_config":
          res = await window.electronAPI.resetInstanceConfig(
            crashData.instanceName,
          );
          break;
        case "open_url":
      ...
      if (res.success) {
        ...
        setFixStatus({
          type: "error",
          message: t("crash.fix_error", {
            error: res.error || t("common.unknown_error", "Unknown error"),
          }),
        });
      }
    } catch (err) {
      setFixStatus({
        type: "error",
        message: t("crash.fix_exception", { error: err.message }),
      });
    } finally {
      setIsApplyingFix(false);
    }
  ...
  const handleFixAllOutdated = async () => {
    const outdatedIssues = issues.filter(
      (i) => i.fixAction === "install_compatible_mod",
    );
    ...
    if (failCount === 0) {
      setFixStatus({
        type: "success",
        message: t("crash.status.all_updated_success", { count: successCount }),
      });
      if (onFixApplied) onFixApplied();
    } else {
      setFixStatus({
        type: "error",
        message: t("crash.status.some_updated_failed", {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-card w-full max-w-2xl rounded-xl border border-border shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
          ...
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">
                {crashData.instanceName} •{" "}
                {t(getExitCodeDescription(crashData.exitCode)) as string}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 max-h-[60vh] overflow-y-auto">
          {issues.length > 0 ? (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                {issues.filter((i) => i.fixAction === "install_compatible_mod")
                  .length > 1 && (
                  <button
                      ...
                      <div className="flex flex-wrap gap-2">
                        {issue?.compatibility?.targetMod && (
                          <div className="text-[10px] px-2 py-0.5 bg-primary/10 text-primary border border-primary/20 rounded uppercase font-bold tracking-wider">
                            {t("crash.labels.fix_prefix")}:{" "}
                            {issue.compatibility.targetMod}
                            {issue?.compatibility?.requiredVersion &&
                              ` v${issue.compatibility.requiredVersion}`}
                          </div>
                        )}
                        {Array.isArray(issue?.compatibility?.affectedMods) &&
                          issue.compatibility.affectedMods.length > 0 && (
                            <div className="text-[10px] px-2 py-0.5 bg-red-500/10 text-red-500 border border-red-500/20 rounded uppercase font-bold tracking-wider">
                              {t("crash.labels.cause_prefix")}:{" "}
                              {issue.compatibility.affectedMods.join(", ")}
                            </div>
                          )}
                      </div>
                    </div>
          ...
          {fixStatus && (
            <div
              className={`mt-6 p-4 rounded-xl text-sm font-medium animate-in slide-in-from-top-2 duration-300 ${
                fixStatus.type === "success"
                  ? "bg-green-500/10 text-green-500 border border-green-500/20"
                  : fixStatus.type === "error"
