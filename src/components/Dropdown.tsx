  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        className={`w-full bg-card border border-border rounded-xl p-3 text-left flex justify-between items-center transition-colors ${disabled ? "opacity-50 cursor-not-allowed" : "hover:bg-accent/40"} ${isOpen ? "border-primary ring-1 ring-primary" : ""}`}
        disabled={disabled}
      >
        <span
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-card border border-border rounded-xl max-h-60 overflow-y-auto custom-scrollbar">
          {options.length === 0 ? (
