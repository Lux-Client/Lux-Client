...
export default function GuideOverlay({
  ...
  return (
    <div className="fixed inset-0 z-[10030] pointer-events-none">
      ...
      <Card className="pointer-events-auto absolute bottom-6 left-1/2 w-[min(760px,calc(100%-2rem))] -translate-x-1/2 border-stroke/70 bg-[#0a0a0a]/95 shadow-2xl backdrop-blur-xl">
        <CardContent className="p-4 sm:p-6">
          <div className="mb-3 flex items-start justify-between gap-3">
            <div className="space-y-2">
              <div className="inline-flex items-center gap-2 rounded-full border border-stroke bg-canvas/60 px-3 py-1 text-[10px] font-medium uppercase tracking-[0.2em] text-muted-foreground">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <Button
              variant="outline"
              onClick={onSkip}
              className="border-stroke/70 bg-canvas/40 text-muted-foreground hover:text-foreground"
            >
              {t("guide.skip")}
            </Button>
            <div className="flex gap-2 sm:justify-end">
              <Button
                ...
                className="border-stroke/70 bg-canvas/40"
              >
                {t("guide.previous")}
              </Button>
              {isLastStep ? (
                <Button onClick={onFinish}>{t("guide.finish")}</Button>
              ) : (
