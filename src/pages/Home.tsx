...
function Home({
  ...
  const loadDashSettings = async () => {
    try {
      ...
      if (res.success) {
        ...
        if (!settings.layout) {
          const newLayout = [];
          if (settings.showRecentInstances !== false)
            newLayout.push({
              id: "recent-instances",
              visible: true,
              width: 12,
          ...
          if (settings.showRecentWorlds !== false)
            newLayout.push({ id: "recent-worlds", visible: true, width: 12 });

          if (settings.showModOfTheDay !== false)
            newLayout.push({ id: "mod-of-the-day", visible: true, width: 12 });

          if (settings.showModpacks !== false)
            newLayout.push({ id: "modpacks", visible: true, width: 12 });

          const existingIds = newLayout.map((i) => i.id);
          if (!existingIds.includes("recent-instances"))
            newLayout.push({
              id: "recent-instances",
              visible: false,
              width: 12,
          ...
          if (!existingIds.includes("recent-worlds"))
            newLayout.push({ id: "recent-worlds", visible: false, width: 12 });

          if (!existingIds.includes("mod-of-the-day"))
            newLayout.push({ id: "mod-of-the-day", visible: false, width: 12 });

          if (!existingIds.includes("modpacks"))
            newLayout.push({ id: "modpacks", visible: false, width: 12 });
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <Dialog
        open={!!selectedModpack}
        onOpenChange={(open) => {
          if (!open) setSelectedModpack(null);
        }}
      >
