...
function Search({ initialCategory, onCategoryConsumed }) {
  ...
  const liveSearchTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const handlePrevImage = (e) => {
    ...
    if (previewProject && previewProject.gallery) {
      setLightboxIndex(
        (prev) =>
          (prev - 1 + previewProject.gallery.length) %
          previewProject.gallery.length,
      );
    }
  };
  useEffect(() => {
  ...
  const handlePreview = async (mod) => {
    try {
      ...
      addNotification(
        t("search.loading_preview", { title: mod.title }),
        "info",
      );
      ...
      if (res.success) {
        ...
        addNotification(
          t("search.failed_preview", { error: res.error }),
          "error",
        );
      }
    } catch (e) {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (projectType === "modpack" && nextOffset === 0 && !query) {
          try {
            const gCraftRes =
              await window.electronAPI.getModrinthProject("oMskb4v5");
            if (gCraftRes.success) {
              ...
              finalResults = [
                gCraft,
                ...finalResults.filter((r) => r.project_id !== "oMskb4v5"),
              ];
            }
          } catch (err) {
        ...
        addNotification(
          t("search.search_failed", { error: res.error }),
          "error",
        );
      }
    } catch (err) {
      addNotification(
        t("search.search_error", { error: err.message }),
        "error",
      );
    } finally {
      setLoading(false);
  ...
  const checkInstanceInstalled = async () => {
    try {
      ...
      if (projectType === "mod") {
        ...
        if (res.success) {
          const ids = new Set(
            res.mods.filter((m) => m.projectId).map((m) => m.projectId),
          );
        ...
        if (res.success) {
          const ids = new Set(
            res.packs.filter((p) => p.projectId).map((p) => p.projectId),
          );
          setInstanceInstalledIds(ids);
        }
        ...
        if (res.success) {
          const ids = new Set(
            res.shaders.filter((s) => s.projectId).map((s) => s.projectId),
          );
          setInstanceInstalledIds(ids);
        }
      }
  ...
  const handleModpackInstall = async (mod) => {
    ...
    try {
      addNotification(
        t("search.fetching_versions", { title: mod.title }),
        "info",
      );
      ...
      addNotification(
        t("search.starting_install", { title: mod.title }),
        "info",
      );
      const installRes = await window.electronAPI.installModpack(
        primaryFile.url,
        mod.title,
        mod.icon_url,
      );
      ...
      if (installRes.success) {
        addNotification(
          t("search.install_success", { title: mod.title }),
          "success",
        );
        ...
        addNotification(
          t("search.install_failed", { error: installRes.error }),
          "error",
        );
      }
    } catch (e) {
  ...
  const openInstall = async (mod) => {
    ...
    setSelectedInstance(
      launcherInstances.length > 0 ? launcherInstances[0].name : "",
    );
  ...
  const handleInstall = async () => {
    ...
    try {
      addNotification(
        t("search.resolving_deps", { title: selectedMod.title }),
        "info",
      );
      const loaders =
        selectedMod.project_type === "shader" ||
        selectedMod.project_type === "resourcepack" ||
        !instance.loader ||
        instance.loader.toLowerCase() === "vanilla"
          ? []
          : [instance.loader];
      ...
      if (res.success && res.versions.length > 0) {
        ...
        const depRes = await window.electronAPI.resolveDependencies(
          version.id,
          loaders,
          [instance.version],
        );
    ...
    try {
      for (let i = 0; i < installList.length; i++) {
        ...
        if (res.success) {
          ...
          addNotification(
            t("search.failed_install_single", {
              title: item.title,
              error: res.error,
            }),
            "error",
      ...
      addNotification(
        t("search.batch_success", { count: installList.length }),
        "success",
      );
      setShowInstallModal(false);
    ...
    <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto min-h-0 pr-1">
            {loading ? (
              ...
              <EmptyState
                ...
                description={t("search.found_results", {
                  count: 0,
                  type:
                    projectType === "mod"
                      ? t("instance_details.content.mods")
      ...
      <Dialog open={showInstallModal} onOpenChange={setShowInstallModal}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {t("search.install_title", { title: selectedMod?.title })}
            </DialogTitle>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className={
                instanceInstalledIds.has(selectedMod?.project_id)
      ...
      <Dialog
        open={showPreviewModal && !!previewProject}
        onOpenChange={(open) => {
          if (!open) setShowPreviewModal(false);
        }}
      >
        ...
        >
          ...
          >
            <img
              ...
              alt={
                previewProject.gallery[lightboxIndex].title || "Gallery Image"
              }
