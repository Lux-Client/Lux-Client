...
function ClientMods() {
    ...
    const [customAutoInstallMods, setCustomAutoInstallMods] = useState([]);
    ...
    const hydrateProjectTitles = async (projectIds) => {
        const ids = [...new Set((projectIds || []).filter((item) => typeof item === 'string' && item.trim()))];
    ...
    const loadInstalledMods = async (versionMapOverride) => {
    ...
    return (
        <div className="h-full p-8 flex flex-col overflow-hidden">
            <div className="mb-6">
                <h1 className="text-3xl font-bold text-white">{t('client_mods.title', 'Client Mods')}</h1>
                <p className="text-sm text-gray-400 mt-1">{t('client_mods.desc', 'Search Fabric mods, install them to selected client versions, and manage client auto install.')}</p>
            </div>

            <div className="bg-surface/40 border border-white/10 rounded-2xl p-4 mb-4">
                <form onSubmit={handleSearchSubmit} className="flex flex-wrap gap-3 items-center">
                    <input
                        ...
                        className="flex-1 min-w-[220px] bg-black/20 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-primary/60"
                    />
                    ...
                    <select
                        ...
                        className="bg-black/20 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-primary/60"
                    >
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                    <span className="text-xs uppercase tracking-widest text-gray-400 font-bold">{t('client_mods.selected_versions', 'Selected Versions')}</span>
                    <button
                        onClick={() => setSelectedVersions(availableVersions)}
                        type="button"
                        className="px-2 py-1 text-[11px] rounded-md bg-white/5 hover:bg-white/10 text-gray-300 border border-white/10"
                    >
                    ...
                    {availableVersions.map((version) => {
                        ...
                        return (
                            <button
                                ...
                                type="button"
                                className={`px-2.5 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${active ? 'bg-primary/20 text-primary border-primary/40' : 'bg-white/5 text-gray-300 border-white/10 hover:bg-white/10'}`}
                            >
                                {version}
                            </button>
                        );
                    ...
                    {availableVersions.length === 0 && (
                        <span className="text-xs text-gray-500">{t('client_mods.no_versions', 'No client versions found yet.')}</span>
                    )}
                </div>
            </div>

            <div className="flex-1 min-h-0 grid grid-cols-1 xl:grid-cols-2 gap-4">
                <div className="bg-surface/30 border border-white/10 rounded-2xl p-4 flex flex-col min-h-0">
                    <div className="flex items-center justify-between mb-3">
                        <h2 className="text-lg font-bold text-white">{t('client_mods.search_results', 'Search Results')}</h2>
                        <span className="text-xs text-gray-400">{t('search.found_results', { count: totalHits, type: t('instance_details.content.mods', 'Mods') })}</span>
                    </div>
                    ...
                    <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar space-y-3 pr-1">
                        {loadingSearch ? (
                            ...
                            results.map((result) => {
                                ...
                                return (
                                    <div key={projectId || result.slug} className="p-3 rounded-xl border border-white/10 bg-black/10">
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <button
                                                ...
                                                className="px-3 py-2 rounded-lg text-sm font-bold bg-white/10 hover:bg-white/20 text-white disabled:opacity-50 disabled:cursor-not-allowed"
                                            >
                    ...
                    <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between">
                        <button
                            ...
                            className="px-3 py-1.5 rounded-lg text-xs font-bold bg-white/10 hover:bg-white/20 disabled:opacity-40"
                        >
                ...
                <div className="grid grid-rows-2 gap-4 min-h-0">
                    <div className="bg-surface/30 border border-white/10 rounded-2xl p-4 min-h-0 overflow-hidden flex flex-col">
                        <h2 className="text-lg font-bold text-white mb-3">{t('client_mods.auto_install_list', 'Client Auto Install')}</h2>
                        ...
                        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar space-y-2 pr-1">
                            {customAutoInstallMods.length === 0 ? (
                                <div className="text-sm text-gray-500">{t('client_mods.no_custom_mods', 'No custom auto install mods yet.')}</div>
                            ) : (
                    ...
                    <div className="bg-surface/30 border border-white/10 rounded-2xl p-4 min-h-0 overflow-hidden flex flex-col">
                        <h2 className="text-lg font-bold text-white mb-3">{t('client_mods.installed_by_version', 'Installed Client Mods')}</h2>
                        ...
                        <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar space-y-3 pr-1">
                            {loadingInstalledMods ? (
                                ...
                                <div className="text-sm text-gray-500">{t('client_mods.no_versions', 'No client versions found yet.')}</div>
                            ) : (
                                displayVersions.map((version) => {
                                    const versionItems = installedModsByVersion[version]?.items || [];
                                    return (
                                        <div key={version} className="border border-white/10 rounded-xl p-3 bg-black/10">
                                            <div className="text-xs uppercase tracking-widest text-gray-400 font-bold mb-2">{version}</div>

                                            {versionItems.length === 0 ? (
                                                <div className="text-sm text-gray-500">{t('client_mods.no_installed_for_version', 'No mods installed for this version.')}</div>
                                            ) : (
            ...
            <div className="mt-4 flex items-center justify-end">
                <button
                    ...
                    className="px-4 py-2 rounded-xl text-sm font-bold bg-white/10 hover:bg-white/20 text-gray-100"
                >
