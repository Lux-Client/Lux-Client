...
import {
  getDefaultStartupValueForMode,
  getStartupModes,
  normalizeStartupPageValue,
} from "../lib/startupPages";
...
function Settings({ mode = "default", onRestartGuide = null }) {
  ...
  const animationPresetOptions = [
    {
      ...
      description: t(
        "settings.general.animations.presets.cinematic_desc",
        "Soft blur, subtle depth and a premium page swap.",
      ),
    },
    {
      id: "glide",
      ...
      description: t(
        "settings.general.animations.presets.glide_desc",
        "A horizontal slide that feels quick and clean.",
      ),
    },
    {
      id: "fade",
      ...
      description: t(
        "settings.general.animations.presets.fade_desc",
        "Minimal crossfade with almost no motion.",
      ),
    },
    {
      id: "zoom",
      ...
      description: t(
        "settings.general.animations.presets.zoom_desc",
        "Pages ease in with a tighter zoom effect.",
      ),
    },
    {
      id: "lift",
      ...
      description: t(
        "settings.general.animations.presets.lift_desc",
        "Content rises in and exits upward with more motion.",
      ),
    },
  ];
  ...
  const [externalLauncherPathInput, setExternalLauncherPathInput] =
    useState("");
  const normalizedStartupPage = normalizeStartupPageValue(
    settings.startPage,
    startupPageOptions,
  );
  const selectedStartupModeId =
    normalizedStartupPage.split(":")[0] || "launcher";
  const selectedStartupMode =
    startupModes.find(
      (startupMode) => startupMode.id === selectedStartupModeId,
    ) ?? startupModes[0];
  ...
  const [searchingAutoInstallMods, setSearchingAutoInstallMods] =
    useState(false);
  const [autoInstallModsSearchResults, setAutoInstallModsSearchResults] =
    useState([]);
  const [autoInstallModsMetadata, setAutoInstallModsMetadata] = useState({});
  const [autoInstallModsListSearch, setAutoInstallModsListSearch] =
    useState("");
  ...
  const handleDeleteRuntime = async (dirPath) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        addNotification(
          t("settings.java.delete_failed", { error: res.error }),
          "error",
        );
      }
    } catch (e) {
  ...
  const loadSettings = async () => {
    ...
    if (res.success) {
      ...
      loadedSettings.startPage = normalizeStartupPageValue(
        loadedSettings.startPage,
        startupPageOptions,
      );
  ...
  const handleCloudLogin = async (providerId) => {
    try {
      ...
      if (res.success) {
        addNotification(
          t("settings.cloud.login_success", {
  ...
  const handleCloudLogout = async (providerId) => {
    try {
      ...
      if (res.success) {
        addNotification(
          t("settings.cloud.logout_success", {
            provider: providerId.replace("_", " "),
          }),
          "success",
        );
  ...
  const handleStartupModeChange = (modeId) => {
    handleChange(
      "startPage",
      getDefaultStartupValueForMode(modeId, startupPageOptions),
    );
  };
  ...
  const handleUninstallLauncher = async () => {
    ...
    if (!result?.success) {
      addNotification(
        result?.error || t("settings.instance.uninstall_failed"),
        "error",
      );
    }
  };
  ...
  const handleBrowseJava = async () => {
    ...
    if (
      ...
      (selectedPath.toLowerCase().endsWith(".exe") ||
        selectedPath.toLowerCase().endsWith(".bin"))
    ) {
  ...
  const handleAddExternalLauncherPath = (pathValue) => {
    ...
    const currentPaths = normalizeExternalLauncherPaths(
      settings.externalLauncherPaths,
    );
    if (currentPaths.includes(normalizedPath)) {
      addNotification(
        t("settings.instance.external_paths_exists", "Path already exists."),
        "warning",
      );
      return;
    }
  ...
  const addAutoInstallMod = async () => {
    ...
    let modName = input;
    const foundInSearch = autoInstallModsSearchResults.find(
      (m) => m.project_id === input,
    );
    if (foundInSearch) {
  ...
  const removeAutoInstallMod = (modId) => {
    const newAutoInstallMods = (settings.autoInstallMods || []).filter(
      (m) => m !== modId,
    );
    handleChange("autoInstallMods", newAutoInstallMods);
  ...
  useEffect(() => {
    const hydrateAutoInstallMetadata = async () => {
      const entries = Array.isArray(settings.autoInstallMods)
        ? settings.autoInstallMods
        : [];
      const missing = entries.filter(
        (entry) => !autoInstallModsMetadata[entry],
      );
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="space-y-5">
            ...
            <CardContent className="space-y-5">
                ...
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex-1 min-w-[200px]">
                    <Label className="text-foreground">
                      {t("settings.general.startup_page")}
                    </Label>
                    <p className="text-xs text-muted-foreground mt-1">
                      {t("settings.general.startup_page_desc")}
                    </p>
                  </div>
                      ...
                      >
                        ...
                        <SelectContent>
                          {startupModes.map((startupMode) => (
                            <SelectItem
                              key={startupMode.id}
                              value={startupMode.id}
                            >
                              {t(
                                startupMode.titleKey,
                                startupMode.titleFallback,
                              )}
                            </SelectItem>
                          ))}
                        </SelectContent>
              ...
              <div className="flex items-center justify-between gap-4 flex-wrap">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="en_uk">
                      {t("settings.general.english_uk")}
                    </SelectItem>
                    <SelectItem value="de_de">
                      {t("settings.general.german")}
                    </SelectItem>
                    <SelectItem value="de_ch">
                      {t("settings.general.swiss_german")}
                    </SelectItem>
                    <SelectItem value="es_es">
                      {t("settings.general.spanish")}
                    </SelectItem>
                    <SelectItem value="fr_fr">
                      {t("settings.general.french")}
                    </SelectItem>
                    <SelectItem value="it_it">
                      {t("settings.general.italian")}
                    </SelectItem>
                    <SelectItem value="pl_pl">
                      {t("settings.general.polish")}
                    </SelectItem>
                    <SelectItem value="pt_br">
                      {t("settings.general.portuguese_br")}
                    </SelectItem>
                    <SelectItem value="pt_pt">
                      {t("settings.general.portuguese_pt")}
                    </SelectItem>
                    <SelectItem value="ro_ro">
                      {t("settings.general.romanian")}
                    </SelectItem>
                    <SelectItem value="ru_ru">
                      {t("settings.general.russian")}
                    </SelectItem>
                    <SelectItem value="sk_sk">
                      {t("settings.general.slovak")}
                    </SelectItem>
                    <SelectItem value="sl_si">
                      {t("settings.general.slovenian")}
                    </SelectItem>
                    <SelectItem value="sv_se">
                      {t("settings.general.swedish")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-4">
                <ToggleBox
                  ...
                  label={t(
                    "settings.general.animations.title",
                    "Page Animations",
                  )}
                  description={t(
                    "settings.general.animations.desc",
                    "Animate page and tab switches instead of switching instantly.",
                  )}
                />
                ...
                {settings.pageAnimationsEnabled !== false && (
                  <div className="rounded-xl border border-stroke bg-muted/20 p-4">
                    <div className="mb-3">
                      <Label className="text-foreground">
                        {t(
                          "settings.general.animations.style_label",
                          "Animation Style",
                        )}
                      </Label>
                      <p className="mt-1 text-xs text-muted-foreground">
                        {t(
                          "settings.general.animations.style_desc",
                          "Choose how pages should move when you switch tabs or sections.",
                        )}
                      </p>
                    </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                      {animationPresetOptions.map((option) => {
                        const isActive =
                          (settings.pageAnimationPreset || "cinematic") ===
                          option.id;
                  ...
                  >
                    {isInstallingJava ? (
                      <>
                        ...
                        <span>
                          {javaInstallProgress
                            ? `${Math.round(javaInstallProgress.progress)}%`
                          ...
                          <div className="flex items-center gap-2">
                            {settings.javaPath === runtime.path ? (
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  handleChange("javaPath", runtime.path)
                                }
                              >
                            ...
                            <Button
                              ...
                              onClick={() =>
                                handleDeleteRuntime(runtime.dirPath)
                              }
                            >
            ...
            <CardContent className="space-y-5">
              <div>
                <Label className="mb-2 block">
                  {t("settings.memory.java_profile")}
                </Label>
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="performance">
                      Performance (Aikar's Flags)
                    </SelectItem>
                    <SelectItem value="low-end">
                      Low-End PC (Aggressive GC)
                    </SelectItem>
                    <SelectItem value="zgc">
                      ZGC (Stable FPS - Java 17+)
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleChange("minMemory", parseInt(e.target.value) || 0)
                    }
                    className="font-mono"
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleChange("maxMemory", parseInt(e.target.value) || 0)
                    }
                    className="font-mono"
                  />
                </div>
              ...
              <div>
                ...
                <div className="flex justify-between text-xs text-muted-foreground mt-2 font-mono">
                  ...
                  <span className="text-primary font-bold">
                    {Math.floor((settings.maxMemory / 1024) * 10) / 10} GB
                  </span>
                  <span>16 GB</span>
                </div>
              </div>
          ...
          {!isClientSettings && (
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <Label className="mb-2 block">
                    {t("settings.instance.storage_path", "Instance Folder")}
                  </Label>
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <Input
                      value={settings.instancesPath || ""}
                      onChange={(e) =>
                        handleChange("instancesPath", e.target.value)
                      }
                      placeholder={t(
                        "settings.instance.storage_path_placeholder",
                        "Default: %appdata%\\Lux\\instances",
                      )}
                      className="flex-1 font-mono text-xs"
                    />
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={handleSelectInstancesPath}
                    >
                      <FolderOpen />
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    {t(
                      "settings.instance.storage_path_desc",
                      "Leave empty to use the default folder. Restart the launcher after changing this path.",
                    )}
                  </p>
                </div>
                ...
                <div>
                  <Label className="mb-2 block">
                    {t(
                      "settings.instance.external_paths_label",
                      "External Launcher Paths",
                    )}
                  </Label>
                  <p className="text-xs text-muted-foreground mb-3">
                    {t(
                      "settings.instance.external_paths_desc",
                      "Add custom folders where Modrinth/CurseForge profiles are stored if you installed them outside default locations.",
                    )}
                  </p>
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <Input
                      ...
                      onChange={(e) =>
                        setExternalLauncherPathInput(e.target.value)
                      }
                      placeholder={t(
                        "settings.instance.external_paths_placeholder",
                        "e.g. D:\\Launchers\\Modrinth\\profiles",
                      )}
                      className="flex-1 font-mono text-xs"
                    />
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={handleBrowseExternalLauncherPath}
                    >
                      <FolderOpen />
                      {t("settings.java.browse", "Browse")}
                    </Button>
                    <Button
                      variant="default"
                      size="sm"
                      onClick={handleAddExternalLauncherPathFromInput}
                    >
                      <Plus />
                      {t("common.add", "Add")}
                    </Button>
                  ...
                  <div className="mt-3 space-y-2">
                    {normalizeExternalLauncherPaths(
                      settings.externalLauncherPaths,
                    ).length === 0 && (
                      <p className="text-xs text-muted-foreground">
                        {t(
                          "settings.instance.external_paths_empty",
                          "No custom external paths configured.",
                        )}
                      </p>
                    )}
                    {normalizeExternalLauncherPaths(
                      ...
                      >
                        ...
                        <Button
                          ...
                          className="h-7 px-2"
                          onClick={() =>
                            handleRemoveExternalLauncherPath(externalPath)
                          }
                        >
                ...
                <ToggleBox
                  ...
                  description={t(
                    "settings.instance.show_modrinth_instances_desc",
                  )}
                />
                ...
                <ToggleBox
                  ...
                  description={t(
                    "settings.instance.show_curseforge_instances_desc",
                  )}
                />
                ...
                {settings.copySettingsEnabled && (
                  <div>
                    <Label className="mb-2 block">
                      {t("settings.instance.source_instance")}
                    </Label>
                    <Select
                      value={settings.copySettingsSourceInstance || ""}
                      onValueChange={(value) =>
                        handleChange("copySettingsSourceInstance", value)
                      }
                    >
                      <SelectTrigger>
          ...
          <Card>
            ...
            <CardContent className="space-y-0">
              ...
              <ToggleBox
                ...
                description={t(
                  "settings.integration.focus_mode_desc",
                  "Disables resource-intensive UI elements like skin animations.",
                )}
              />
              <ToggleBox
                className="mt-4 pt-4 border-t border-stroke"
                ...
                label={t(
                  "settings.integration.minimize_to_tray",
                  "Minimize to Tray",
                )}
                description={t(
                  "settings.integration.minimize_to_tray_desc",
                  "Hide the launcher to the system tray when closing or minimizing.",
                )}
              />
              {window.electronAPI &&
                window.electronAPI.platform === "win32" && (
                  <ToggleBox
                    ...
                    label={t(
                      "settings.integration.minimal_mode",
                      "Minimal Mode",
                    )}
                    description={t(
                      "settings.integration.minimal_mode_desc",
                      "Automatically minimize the launcher to the taskbar when a game starts.",
                    )}
                  />
                )}
              <ToggleBox
          ...
          {!isClientSettings && settings.enableAutoInstallMods && (
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  {t("settings.auto_install.management_desc")}
                </p>

                <div>
                  <Label className="mb-2 block">
                    {t("settings.auto_install.add_label")}
                  </Label>
                      ...
                      {autoInstallModsSearchResults.map((mod) => (
                        ...
                        >
                          ...
                          <div className="flex flex-wrap gap-1 mt-0.5">
                            {getSourceTags(mod.source, mod.sources).map(
                              (sourceTag) => (
                                <span
                                  key={`${mod.project_id}-${sourceTag}`}
                ...
                {(settings.autoInstallMods || []).length > 0 ? (
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <Label>
                        {t("settings.auto_install.count_label", {
                          count: settings.autoInstallMods.length,
                        })}
                      </Label>
                    </div>
                    <Input
                      ...
                      onChange={(e) =>
                        setAutoInstallModsListSearch(e.target.value)
                      }
                      placeholder={t(
                        "settings.auto_install.list_search_placeholder",
                      )}
                      className="mb-3"
                    />
                    <div className="space-y-2">
                      {(settings.autoInstallMods || [])
                        .filter((mod) => {
                          ...
                          const searchQuery =
                            autoInstallModsListSearch.toLowerCase();
                          return (
                            modName.toLowerCase().includes(searchQuery) ||
                            mod.toLowerCase().includes(searchQuery)
                          );
                        })
                        .map((mod) => (
                          <div
                          ...
                          >
                            <div>
                              <p className="text-sm text-foreground font-medium">
                                {autoInstallModsMetadata[mod] || mod}
                              </p>
                      ...
                      {autoInstallModsListSearch &&
                        (settings.autoInstallMods || []).filter((mod) => {
                          ...
                          <div className="text-center py-4 text-muted-foreground text-sm">
                            {t("settings.auto_install.no_matches")}
                          </div>
                        )}
                    </div>
                  </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-0">
              <ToggleBox
                ...
                label={t(
                  "settings.compatibility.low_graphics",
                  "Low Graphics Mode",
                )}
                description={t(
                  "settings.compatibility.low_graphics_desc",
                  "Disables resource-intensive 3D previews (e.g. skin preview) to improve performance on older hardware.",
                )}
              />
              <ToggleBox
                className="mt-4 pt-4 border-t border-stroke"
                ...
                label={t(
                  "settings.compatibility.legacy_gpu",
                  "Legacy GPU Support",
                )}
                description={t(
                  "settings.compatibility.legacy_gpu_desc",
                  "Disables hardware acceleration and uses basic OpenGL. Enable this if you experience crashes or black screens. (Requires App Restart)",
                )}
              />
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-5">
                ...
                {[
                  {
                    id: "GOOGLE_DRIVE",
                    name: "Google Drive",
                    icon: "assets/cloud-backup/drive.svg",
                  },
                  {
                    id: "DROPBOX",
                    name: "Dropbox",
                    icon: "assets/cloud-backup/dropbox.svg",
                  },
                ].map((provider) => (
                  <div
                    key={provider.id}
                      ...
                      <div className="space-y-2">
                        <div className="text-xs text-muted-foreground">
                          <p className="font-medium text-foreground truncate">
                            {cloudStatus[provider.id].user?.name}
                          </p>
                          <p className="truncate">
                            {cloudStatus[provider.id].user?.email}
                          </p>
                        </div>
                        <Button
                          variant="destructive"
              ...
              <div className="space-y-4">
                <ToggleBox
                  ...
                  onChange={(val) =>
                    handleChange("cloudBackupSettings", {
                      ...settings.cloudBackupSettings,
                      enabled: val,
                    })
                  }
                ...
                {settings.cloudBackupSettings?.enabled && (
                  <div className="ml-10 space-y-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <Label className="text-foreground">
                          {t("settings.cloud.default_provider")}
                        </Label>
                        <p className="text-xs text-muted-foreground">
                          {t("settings.cloud.default_provider_desc")}
                        </p>
                      </div>
                      <Select
                        value={
                          settings.cloudBackupSettings?.provider ||
                          "GOOGLE_DRIVE"
                        }
                        onValueChange={(value) =>
                          handleChange("cloudBackupSettings", {
                    ...
                    <ToggleBox
                      checked={
                        settings.cloudBackupSettings?.autoRestore || false
                      }
                      onChange={(val) =>
                        handleChange("cloudBackupSettings", {
                          ...settings.cloudBackupSettings,
                ...
                >
                  ...
                  <span>
                    {isCheckingUpdate
                      ...
                      : t("settings.update.check_btn", "Check for Updates")}
                  </span>
                </Button>
              </div>
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-semibold text-foreground">
                        {t("settings.update.available", "Update Available!")}
                      </h3>
                  ...
                  {isDownloadingUpdate && (
                    <div className="mt-3">
                      <div className="flex justify-between text-xs text-muted-foreground mb-1.5">
                        <span>
                          {t("settings.update.downloading", "Downloading...")}
                        </span>
                    ...
                    <>
                      ...
                      <div>
                        <h4 className="text-xs font-semibold text-muted-foreground uppercase mb-2">
                          {t("settings.update.changelog", "Release Notes")}
                        </h4>
                ...
                <AccordionContent className="px-5 pb-5">
                  <div className="space-y-5">
                    {isFeatureEnabled("settingsDevelopmentTesting") && (
                      <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-4">
                        ...
                        <p className="text-[10px] text-muted-foreground mt-2">
                          Overrides local version string for update check
                          simulation.
                        </p>
                      </div>
                    )}
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            <p className="font-medium text-foreground text-sm">
                              {t("settings.maintenance.troubleshooting_title")}
                            </p>
                            <p className="text-xs text-muted-foreground mt-0.5">
                              {t("settings.maintenance.troubleshooting_desc")}
                            </p>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="rounded-lg border border-destructive/20 bg-destructive/5 p-4 flex flex-col justify-between min-h-[140px]">
                          <div>
                            <h3 className="font-semibold text-destructive text-sm">
                              {t("settings.maintenance.factory_reset_title")}
                            </h3>
                            <p className="text-xs text-muted-foreground mt-2">
                    ...
                    <div className="rounded-lg border border-destructive/25 bg-destructive/5 p-4">
                      <h3 className="font-semibold text-foreground text-sm">
                        {t("settings.instance.uninstall_title")}
                      </h3>
      ...
      {showRestartModal && (
        <ConfirmationModal
          ...
          message={t(
            "settings.compatibility.restart_msg",
            "Enabling Legacy GPU Support requires an application restart to apply changes to the graphics engine. Would you like to restart now?",
          )}
          confirmText={t(
            "settings.compatibility.restart_confirm",
            "Restart Now",
          )}
