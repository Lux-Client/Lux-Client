  ...
  return (
    <div className="h-screen w-screen flex flex-col items-center justify-center bg-[#0a0a0a] overflow-hidden relative">
      ...
      <motion.div
        ...
        className="z-10 max-w-lg w-full p-10 rounded-[2rem] border border-white/5 bg-white/[0.02] backdrop-blur-3xl shadow-2xl flex flex-col items-center text-center"
      >
        <div className="relative mb-8">
          <div className="absolute inset-0 bg-destructive/20 blur-2xl rounded-full scale-150 animate-pulse" />
          <div className="relative bg-destructive/10 p-5 rounded-3xl border border-destructive/20 group-hover:scale-110 transition-transform duration-500">
            <TriangleAlert
        ...
        <h1 className="text-3xl font-extrabold mb-3 tracking-tight text-white bg-clip-text text-transparent bg-gradient-to-b from-white to-white/60">
        ...
        <div className="flex flex-col w-full gap-3">
          <button
            ...
            className="group relative flex items-center justify-center gap-2 bg-primary text-black h-14 rounded-2xl font-bold overflow-hidden transition-all hover:scale-[1.02] active:scale-95 shadow-[0_0_20px_rgba(var(--primary-color-rgb),0.3)]"
          >
...
const normalizeThemeSchema = (theme: any = {}) => ({
  ...
  ...theme,
  primaryColor:
    theme.primaryColor || theme.primary || DEFAULT_THEME.primaryColor,
  backgroundColor:
  ...
  surfaceColor:
    theme.surfaceColor || theme.surface || DEFAULT_THEME.surfaceColor,
  sidebarColor:
    typeof theme.sidebarColor === "string"
      ? theme.sidebarColor
      : typeof theme.sidebar === "string"
        ? theme.sidebar
        : "",
  textOnBackground:
  ...
  textOnSurface:
    theme.textOnSurface || theme.text || DEFAULT_THEME.textOnSurface,
...
function App() {
  ...
  const [isInstallingRequiredJava, setIsInstallingRequiredJava] =
    useState(false);
  const [requiredJavaInstallError, setRequiredJavaInstallError] = useState("");
  const [isCommandPaletteOpen, setIsCommandPaletteOpen] = useState(false);
  const [guidePromptMode, setGuidePromptMode] = useState<GuideMode | null>(
    null,
  );
  const [guidePromptDoNotShowAgain, setGuidePromptDoNotShowAgain] =
    useState(false);
  ...
  const guidePromptShownThisSessionRef = useRef<Record<GuideMode, boolean>>({
    ...GUIDE_PROMPT_SESSION_DEFAULTS,
  });
  const navigationHistoryRef = useRef([
    { mode: "launcher", view: "dashboard" },
  ]);
  ...
  useEffect(() => {
    ...
    const activeEntry =
      navigationHistoryRef.current[navigationIndexRef.current];
    if (
      activeEntry?.mode === currentMode &&
      activeEntry?.view === currentView
    ) {
      syncNavigationButtons();
      return;
    }

    const truncatedHistory = navigationHistoryRef.current.slice(
      0,
  ...
  const resolveFontFamily = (nextTheme) => {
    ...
    const customFonts = (nextTheme.customFonts ?? []).map(
      (font) => font.family,
    );
    const availableFonts = new Set([...builtInFonts, ...customFonts]);
    return availableFonts.has(nextTheme.fontFamily)
  ...
  const saveGuidePromptPreference = async (
    mode: GuideMode,
    enabled: boolean,
  ) => {
  ...
  useEffect(() => {
    if (currentMode === "launcher" && currentView !== "instance-details") {
      lastClientView.current = resolveModeView(
        "launcher",
        currentView,
        startupPageOptions,
      );
    ...
    if (currentMode === "server" && currentView !== "server-details") {
      lastServerView.current = resolveModeView(
        "server",
        currentView,
        startupPageOptions,
      );
    ...
    if (currentMode === "tools") {
      lastToolsView.current = resolveModeView(
        "tools",
        currentView,
        startupPageOptions,
      );
  ...
  useEffect(() => {
    ...
    const removeThemeListener = window.electronAPI?.onThemeUpdated(
      (newTheme) => {
    ...
    const removeStatusListener = window.electronAPI?.onInstanceStatus(
      ({ instanceName, status, loader, version }) => {
        setRunningInstances((prev) => {
          const next = { ...prev };
          if (status === "stopped" || status === "deleted") {
            delete next[instanceName];
            if (status === "stopped")
              Analytics.updateStatus(false, instanceName, {
                loader,
                version,
                mode: currentMode,
              });
            ...
            if (status === "running")
              Analytics.updateStatus(true, instanceName, {
                loader,
                version,
                mode: currentMode,
              });
    ...
    const removeServerStatusListener = window.electronAPI.onServerStatus?.(
      ({ serverName, status }) => {
        setRunningInstances((prev) => {
          const next = { ...prev };
          if (
    ...
    const removeInstallListener = window.electronAPI?.onInstallProgress(
      ({ instanceName, progress, status, type }) => {
        setActiveDownloads((prev) => {
          const next = { ...prev };
          if (progress >= 100) {
            delete next[instanceName];
            ...
            next[instanceName] = {
              progress: progress || prev[instanceName]?.progress || 0,
              status,
              type: type || "install",
            };
          }
    ...
    const removeLaunchProgressListener = window.electronAPI?.onLaunchProgress(
      (e) => {},
    );

    const removeWindowStateListener = window.electronAPI?.onWindowStateChange(
      (maximized) => {
        setIsMaximized(maximized);
      },
    );

    const removeCrashReportListener = window.electronAPI?.onCrashReport(
      (data) => {
        if (appSettingsRef.current?.enableSmartLogAnalytics !== false) {
          ...
          console.log(
            "[App] Crash detected but Smart Log Analytics is disabled.",
          );
        }
      },
    );

    const removeJavaRequiredListener = window.electronAPI?.onJavaRequired(
      (data) => {
  ...
  const handleInstallRequiredJava = async () => {
    ...
    try {
      ...
      if (!installRes?.success || !installRes?.path) {
        setRequiredJavaInstallError(
          installRes?.error ||
            `Java ${version} konnte nicht installiert werden.`,
        );
        return;
      }

      const settingsRes = await window.electronAPI.getSettings();
      const currentSettings = settingsRes?.success
      ...
      if (saveRes?.success) {
        ...
        setRequiredJavaInstallError(
          saveRes?.error ||
            "Java wurde installiert, aber der Pfad konnte nicht gespeichert werden. Bitte in Settings setzen.",
        );
      }
    } catch (e: any) {
      setRequiredJavaInstallError(
        e?.message || "Unbekannter Fehler bei der Java-Installation.",
      );
    } finally {
      setIsInstallingRequiredJava(false);
    }
  ...
  const handleThemeModeSelect = async (mode) => {
    const selectedThemePreset =
      mode === "light" ? LIGHT_LUX_THEME_PRESET : LUX_FOREST_THEME_PRESET;
    const nextTheme = {
      ...(appSettings.theme || {}),
      ...selectedThemePreset,
    };
  ...
  const handleBackToDashboard = () => {
    ...
    startTransition(() => {
      setCurrentView(
        currentMode === "launcher" ? "dashboard" : "server-dashboard",
      );
    });
  };
  ...
  const handleModeSelect = (mode) => {
    ...
    if (mode === "launcher") {
      setCurrentView(
        resolveModeView("launcher", lastClientView.current, startupPageOptions),
      );
    } else if (mode === "server") {
      setCurrentView(
        resolveModeView("server", lastServerView.current, startupPageOptions),
      );
    } else if (mode === "client") {
      setCurrentView(
        resolveModeView("client", "open-client", startupPageOptions),
      );
    } else if (mode === "tools") {
      setCurrentView(
        resolveModeView("tools", lastToolsView.current, startupPageOptions),
      );
    }
    setSelectedInstance(null);
    setSelectedServer(null);
  ...
  const guideSteps = React.useMemo(
    () => getGuideSteps(guideMode, { canAccessSkins }),
    [guideMode, canAccessSkins],
  );
  ...
  const isCommandPaletteAvailable =
    !isGuidePromptBlockedBySetup && !isGuideRunning && guidePromptMode === null;
  const pageAnimationsEnabled = appSettings.pageAnimationsEnabled !== false;
  const pageAnimationPreset =
    typeof appSettings.pageAnimationPreset === "string"
      ? appSettings.pageAnimationPreset
      : DEFAULT_PAGE_ANIMATION_PRESET;
  const activePageTransitionPreset =
    getPageTransitionPreset(pageAnimationPreset);
  ...
  const renderCurrentPage = () => {
    if (currentMode === "launcher") {
      ...
      if (currentView === "library") {
        return (
          <Dashboard
            ...
            runningInstances={runningInstances}
            activeDownloads={activeDownloads}
            triggerCreate={triggerCreateInstance}
            onCreateHandled={() => setTriggerCreateInstance(false)}
            isGuest={isGuest}
          />
        );
      }
      ...
      if (currentView === "search") {
        return (
          <Search
            ...
            onCategoryConsumed={() => setSearchCategory(null)}
          />
        );
      }
      ...
      if (currentView === "skins" && !isGuest) {
        return (
          <Skins onLogout={handleLogout} onProfileUpdate={setUserProfile} />
        );
      }
      ...
      if (currentView === "settings") {
        return (
          <Settings
            mode="launcher"
            onRestartGuide={() => handleRestartGuide("launcher")}
          />
        );
      }
    ...
    if (currentMode === "client" && isFeatureEnabled("openClientPage")) {
      ...
      if (currentView === "settings") {
        return (
          <Settings
            mode="client"
            onRestartGuide={() => handleRestartGuide("client")}
          />
        );
      }
    }
    ...
    if (currentMode === "tools") {
      ...
      if (currentView === "settings") {
        return (
          <Settings
            mode="tools"
            onRestartGuide={() => handleRestartGuide("tools")}
          />
        );
      }
    }
  ...
  return (
    <ExtensionProvider>
                  ...
                  <AnimatePresence mode="wait" initial={false}>
                    {currentPage && (
                      <motion.div
                        ...
                        data-page-animation={
                          shouldAnimatePages ? pageAnimationPreset : "none"
                        }
      ...
      <ExtensionSlot
        name="app.overlay"
        className="absolute inset-0 pointer-events-none z-[9999] *:pointer-events-auto"
      />
