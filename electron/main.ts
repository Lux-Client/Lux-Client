...
const { isPathInside } = require('../backend/utils/path-safety');
const { calculateFileSha256, resolveExpectedReleaseSha256 } = require('../backend/utils/hash-utils');
const { fetchAndVerifyManifest, verifyArtifactHash } = require('../backend/utils/manifest-verify');
    ...
    const performCheck = async () => {
        ...
        try {
            ...
            if (needsUpdate) {
                ...
                // Fetch and verify release manifest before downloading
                let verifiedManifest = null;
                try {
                    verifiedManifest = await fetchAndVerifyManifest(axios, release);
                    console.log(`[Main] Manifest verified for version ${verifiedManifest.version} with ${Object.keys(verifiedManifest.artifacts).length} artifacts`);
                } catch (manifestError) {
                    console.error('[Main] Manifest verification failed:', manifestError.message);
                    sendSplashStatus({ status: 'Starting', detail: 'Update manifest verification failed. Launching app...' });
                    setTimeout(launchMain, 1500);
                    return;
                }
                ...
                if (asset) {
                    ...
                    // Verify artifact hash from release manifest
                    if (verifiedManifest && verifiedManifest.artifacts) {
                        const manifestResult = await verifyArtifactHash(verifiedManifest, safeAssetName, targetPath);
                        if (!manifestResult.valid) {
                            await fs.remove(targetPath);
                            throw new Error(
                                `Update verification failed: manifest hash mismatch for ${safeAssetName}. ` +
                                (manifestResult.error || `Expected ${manifestResult.expected}, got ${manifestResult.actual}.`)
                            );
                        }
                        console.log(`[Main] Manifest hash verified for ${safeAssetName}: ${manifestResult.actual}`);
                    } else {
...
function setupAppMediaProtocol() {
    protocol.handle('app-media', (request) => {
        try {
            ...
            const isInside = isPathInside(userDataPath, resolvedPath);
